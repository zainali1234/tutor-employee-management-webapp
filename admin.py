# handles all user admin functionalities
...
# renders webpage that allows user to choose what student database action page to be redirected to 
...
# renders all students registered within the database
...
# prompts user to enter information about a student to be entered into the database
@admin.route('/account/register-student', methods=['GET','POST'])
@login_required
def register_student():
    # if user submits student information, then check conditions to make sure requested account info is valid
    if request.method == 'POST':
        ...
        if len(student_name) < 3:
            ...
            # if all conditions are met, new_student object is created with input attributes and is added to the database 
...
# prompts user to choose a student to delete from the database
...
# renders webpage that allows user to choose what session database action page to be redirected to 
...
# renders all sessions registered within the database
...
# prompts user to choose a session to delete from the database
...
# renders webpage that allows user to choose what employee database action page to be redirected to 
...
# renders all employees registered within the database
...
# prompts user to choose an employee to delete from the database
